...
/* 4)   * Create a reference for each input value. 
        * Hint: it might be helpful to give each input an id in the index.html OR
            to use a function that selects all of the inputs at once ...
*/

/* 5) * Once you have each input value, make a callback function onClick() with a parameter called event
      * Within the function: 
            * Make a new object for that hero that the user entered! 
              Make the properties for each input matching the format you used above for the other heroes.
              Add an "img" property to your object and pick one of images in the assets folder to act as a default image
              for user-made heros.
              Add an "abilities" property and set it equal to "[Support]" as the default ability;
            
            * prevent the default settings of refreshing the page using the event parameter.
            * Add the new hero object to the "heroes" array.
            * Create a variable "grid" and use the DOM to store the reference to div in your index.html
                with the id "gridDiv". (NOTE: will only work after all 3 files are complete!)
            * Empty the grid div container by setting the innerHTML to an empty string.
            * Use the function call " heroCreator.populateGrid(gridDiv,heroes); "
*/

// 6) At the bottom of "events.js", give the "Submit" button an on click event handler that uses the callback function;




/*
BONUS 1: add 3 more Overwatch heroes from Overwatch (remember to add 
them to the "heroes" array!)

BONUS 2: In "style.css", style your form to look more like the rest
of the website
*/
